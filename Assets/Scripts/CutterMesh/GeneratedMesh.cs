...
public class GeneratedMesh
{
    ...
    public int[] Indices(int subIndice)
    {
        if (subIndice < submeshIndices.Count)
            return submeshIndices[subIndice].ToArray();
        else
            throw new System.Exception("Indice maior que o array dos submeshs");
    }
}
